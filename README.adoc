...
Please note that there is a rewrite of Unoconv called "Unoserver":
https://github.com/unoconv/unoserver/

We are running Unoserver successfully in production, and it's now the
recommended solution. For the time being Unoconv is in bugfix mode only,
there will be no new features. Please switch to Unoserver.
